...
function renderCameraSettingsView() {
  ...
  if (!cameras.length) {
    return `<div class="empty"><div class="empty-icon">${icone('camera', { taille: 40 })}</div><div class="empty-text">Aucune caméra dans l'inventaire pour l'instant. Importe ta liste (Inventaire) pour pouvoir créer des profils de réglages.</div></div>`;
  }
  ...
  if (!profiles.length) {
    html += `<div class="empty"><div class="empty-icon">${icone('sliders-horizontal', { taille: 40 })}</div><div class="empty-text">Aucun profil pour l'instant — appuie sur "+ Profil" pour en créer un.</div></div>`;
    return html;
  }
  ...
  html += profiles.map(p => `
    <div class="card" style="margin-bottom:14px">
      <div class="card-header">
        ...
        <button class="btn-icon-sm" onclick="openEditCameraProfile('${p.id}')" aria-label="Modifier">${icone('pencil', { taille: 15 })}</button>
      </div>
      <div>
        ${p.settings.map(s => `
          <div class="check-item">
            <div class="check-box ${s.done?'checked':''}" onclick="toggleProfileSetting('${s.id}',${s.done})">${s.done ? icone('check', { taille: 13, trait: 3 }) : ''}</div>
            <div class="check-item-content"><div class="check-item-label ${s.done?'done':''}">${esc(s.text)}</div></div>
            <button class="btn-icon-sm" style="color:var(--red)" onclick="deleteProfileSetting('${s.id}')" aria-label="Supprimer">${icone('x', { taille: 15 })}</button>
          </div>`).join('')}
      ...
      <div style="padding:12px 16px;border-top:1px solid var(--border2)">
        ...
        ${cameras.map(c => `
          <div class="pick-row" onclick="assignCameraProfileFromSettings('${c.id}', '${c.camera_profile_id===p.id ? '' : p.id}')">
            <div class="pick-box ${c.camera_profile_id===p.id?'checked':''}">${icone('check', { taille: 13, trait: 3 })}</div>
            <div style="font-size:14px">${esc(c.designation)}</div>
          </div>`).join('')}
      </div>
      <div style="padding:12px 16px;border-top:1px solid var(--border2)">
        <button class="btn ${p.validated ? 'btn-secondary' : 'btn-primary'}" onclick="validateCameraProfile('${p.id}')">${icone('circle-check', { taille: 16 })} ${p.validated ? 'Confirmé — reconfirmer' : 'Confirmer et insérer dans la Checklist'}</button>
      </div>
    </div>`).join('');
...
function profileColorPicker(selected) {
  return `<div class="note-colors">${PROFILE_COLORS.map(c =>
    `<div class="note-swatch ${c===selected?'sel':''}" style="background:${c};border-color:${c===selected?'var(--text)':'oklch(88% 0.004 90)'}" data-c="${c}" onclick="pickProfileColor('${c}')"></div>`).join('')}
  </div><input type="hidden" id="in-profile-color" value="${esc(selected)}">`;
}
function pickProfileColor(c) {
  document.querySelectorAll('#sheet .note-swatch').forEach(s => { s.classList.toggle('sel', s.dataset.c === c); s.style.borderColor = s.dataset.c === c ? 'var(--text)' : 'oklch(88% 0.004 90)'; });
